		...
		btnRegistrar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if(textField.getText().equals("")){
					JOptionPane.showMessageDialog(null, "Debe buscar la direcci�n de la foto","Alert",JOptionPane.ERROR_MESSAGE);
				}else {
					Path_Photo path = new Path_Photo();
					String patha = "";
					for(int j = 0; j < textField.getText().length(); j++) {
						if(textField.getText().substring(j, j+1).equals("\\")) {
							patha += "\\\\";
						}else {
							patha += textField.getText().substring(j, j+1);
						}
					}
					path.setPath(patha);
					...
					JOptionPane.showMessageDialog(null, "Se registro la direcci�n de las fotos con exito","Extio",JOptionPane.INFORMATION_MESSAGE);
					textField.setText("");
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if(respuesta == JFileChooser.APPROVE_OPTION) {
					...
					textField.setText(archivoElegido.getPath());
				}
			}
		});
